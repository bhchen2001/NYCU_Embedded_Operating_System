...
int handle_confirm(int client_sockfd, Order *orders, int current_restaurant);
...
int handle_confirm(int client_sockfd, Order *orders, int current_restaurant) {
    ...
    // check which deliverman to use
    int delivery_distance = restaurants[current_restaurant].distance;
    int candidate_deliverman = check_deliveryman(restaurants[current_restaurant].distance);

    // send check message to client if distance is too far
    if (deliverman[candidate_deliverman].remaining_time + delivery_distance > 30) {
        char client_message[BUFFER_SIZE];
        memset(client_message, 0, BUFFER_SIZE);
        if (snprintf(response, BUFFER_SIZE, "Your delivery will take a long time, do you want to wait?\n") < 0) {
            perror("error writing to buffer");
            exit(1);
        }
        if (send(client_sockfd, response, BUFFER_SIZE, 0) < 0) {
        ...
        if (strcmp(client_message, "No") == 0) {
            printf("Client canceled the delivery\n");
            return -1;
        } 
        ...
        // printf("[DEBUG] Client accepted the delivery\n");
    }
    ...
    // update deliverman status
    update_deliverman_status(candidate_deliverman, delivery_distance);

    // wait based on restaurant distance (lock delivery mutex)
    pthread_mutex_lock(&deliverman[candidate_deliverman].delivery_mutex);
    sleep(delivery_distance);
    pthread_mutex_unlock(&deliverman[candidate_deliverman].delivery_mutex);
    ...
    // printf("[DEBUG] Delivery has arrived and you need to pay %d$\n", total_price);
